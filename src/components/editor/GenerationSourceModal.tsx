...
type Props = {
  ...
  contextIndexes: ReadonlySet<number>;
  selectedText: string;
...
export function GenerationSourceModal({ open, lines, selectedIndexes, contextIndexes, selectedText, onToggle, onSelectAll, onClear, onCancel, onConfirm }: Props) {
  useEffect(() => {
    if (!open) return;
  ...
  return <div className="source-modal-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}>
    <section className="source-modal" role="dialog" aria-modal="true" aria-labelledby="source-modal-heading">
      <header className="source-modal-header">
        <div><p className="eyebrow">SOURCE RANGE</p><h2 id="source-modal-heading">図に使うメモの範囲</h2><p>選択した行と、階層を保つために必要な親行だけをAIに渡します。</p></div>
      ...
      <div className="source-modal-body">
        <div className="source-line-list" aria-label="図に使うメモの行">
          {lines.map(({ index, text }) => <label className={`source-line ${selectedIndexes.has(index) ? 'selected' : ''} ${contextIndexes.has(index) ? 'context' : ''}`} key={index}>
            <input type="checkbox" checked={selectedIndexes.has(index) || contextIndexes.has(index)} disabled={contextIndexes.has(index)} onChange={() => onToggle(index)} />
            <span className="source-line-number">{String(index + 1).padStart(2, '0')}</span>
            <span className="source-line-text">{text}{contextIndexes.has(index) && <small className="source-context-label">文脈</small>}</span>
          </label>)}
        </div>
        <div className="source-selection-preview"><p className="source-preview-label">AIへ渡す内容（文脈行を含む）</p><pre>{selectedText || '行を1つ以上選択してください。'}</pre></div>
      </div>
